...
Tanker::Tanker(const string& name_, Point position_) 
	: Ship(name_, position_, 100, 10, 2, 0), cargo(0), 
	cargo_capacity(1000), tanker_state(Tanker_state::no_destination){}
...
void Tanker::set_destination_position_and_speed
...
void Tanker::set_destination_island_and_speed
...
void Tanker::update() {
	...
	case Tanker_state::loading: {
		...
		if (fuel_needed < 0.005) {
			...
			(unload_destination, get_maximum_speed());
	...
	case Tanker_state::unloading:
		...
		if (cargo == 0.0) {
			Ship::set_destination_island_and_speed
			(load_destination, get_maximum_speed());
