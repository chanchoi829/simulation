...
Track_base::Track_base(Point position_)
    : position(position_)
    , altitude(0.)
{ }
...
Compass_position Track_base::get_CPA_range_and_bearing_from(const Track_base* track_ptr) const
{
    ...
    Compass_position result = compute_CPA(
        track_ptr->get_course_speed(), course_speed, get_range_and_bearing_from(track_ptr), time_until_CPA);
    return result;
}
