...
Controller::Controller() {
	...
	command_set = { "open_map_view", "close_map_view", "open_sailing_view", 
		"close_sailing_view", "open_local_view", "close_local_view",  
		"default",  "size", "zoom", "pan", "show", "quit", 
		"status", "go", "create", "course", "position", "destination", 
		"load_at", "unload_at", "chain_all", "chain", "unchain", "dock_at", 
		"attack", "refuel", "stop", "stop_attack" , "chain", "unchain", "chain_all"
		"create_group", "add_ship_to_group", "remove_ship_from_group",
		"remove_group", "add_group_to_group", "describe_groups"
	};
}
...
void Controller::run() {
	...
	while (true) {
		try {
			cout << "\nTime " 
				<< Model::get_instance().get_time() << ": Enter command: ";
			cin >> first_input;
			...
			shared_ptr<Ship_component> ship_ptr 
				= Model::get_instance().get_ship_ptr(first_input);
			...
			if (ship_ptr) {
				...
				shared_ptr<Ship_component> composite_ptr =
					Model::get_instance().get_ship_composite_ptr(first_input);
...
void Controller::close_map_view() {
	...
	view_vec.erase(find(view_vec.cbegin(), 
		view_vec.cend(), map_view_ptr));
	map_view_ptr.reset();
}
...
void Controller::close_sailing_view() {
	...
	view_vec.erase(find(view_vec.cbegin(), 
		view_vec.cend(), sailing_view_ptr));
...
void Controller::open_local_view() {
	...
	shared_ptr<Local_view> new_view = 
		make_shared<Local_view>(Local_view(ship_ptr->get_name()));

	if (!local_view_ptr_map.insert(make_pair(ship_ptr->get_name(), 
		new_view)).second)
...
void Controller::view_show() const {
	...
	for_each(view_vec.cbegin(), view_vec.cend(), 
		[](const shared_ptr<View>ptr) {ptr->draw();});
}
...
void Controller::model_create() const{
	...
	Model::get_instance().add_ship
	(create_ship(ship_name, object_type, Point(x, y)));
}
...
void Controller::model_remove_ship_from_composite() const {
	...
	Model::get_instance().
		remove_ship_from_composite(composite_name, ship_name);
}
...
void Controller::model_add_ship_to_composite() const {
	...
	Model::get_instance().add_ship_to_composite
	(composite_name, Model::get_instance().get_ship_ptr(ship_name));
}
...
void Controller::model_add_composite_to_composite() const {
	...
	Model::get_instance().add_composite_to_composite
	(composite_name, create_composite(new_composite_name));
}
...
void Controller::ship_destination
(shared_ptr<Ship_component> const ship_ptr) const{
	string island_name;
	cin >> island_name;

	shared_ptr<Island> island_ptr = 
		Model::get_instance().get_island_ptr(island_name);
...
// basic validity check : Island must exist
void Controller::ship_unload_at
(shared_ptr<Ship_component> const ship_ptr) const {
	string island_name;
	cin >> island_name;
...
void Controller::ship_chain_all_ship
(shared_ptr<Ship_component> const ship_ptr) const {
...
void Controller::ship_chain_ship
(shared_ptr<Ship_component> const ship_ptr) const {
	string ship_name;
	cin >> ship_name;

	shared_ptr<Ship> ship_to_chain =
		Model::get_instance().get_ship_ptr(ship_name);
...
// check: ship_ptr exists
void Controller::ship_unchain_ship
(shared_ptr<Ship_component> const ship_ptr) const {
	string ship_name;
	cin >> ship_name;

	shared_ptr<Ship> ship_to_unchain =
		Model::get_instance().get_ship_ptr(ship_name);
...
void Controller::ship_attack(shared_ptr<Ship_component> const ship_ptr) const {
	...
	shared_ptr<Ship> ship_target_ptr = 
		Model::get_instance().get_ship_ptr(ship_name);
...
// refuel a Ship
void Controller::ship_refuel
(shared_ptr<Ship_component> const ship_ptr) const {
	ship_ptr->refuel();
}

// stop a Ship
void Controller::ship_stop
(shared_ptr<Ship_component> const ship_ptr) const {
	ship_ptr->stop();
}

// stop a Ship from attacking
void Controller::ship_stop_attack
(shared_ptr<Ship_component> const ship_ptr) const {
...
void Controller::check_if_name_valid(const std::string& name) const {
	...
	if (any_of(name.cbegin(), name.cend(),
...
void Controller::process_ship_command
(std::shared_ptr<Ship_component> const ptr) {
