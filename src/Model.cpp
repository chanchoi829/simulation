...
Model::Model() : time(0){
	...
	sim_object_map.insert(*island_map.insert(make_pair
	("Exxon", make_shared<Island>("Exxon", Point(10, 10), 1000, 200))).first);
	sim_object_map.insert(*island_map.insert(make_pair
	("Shell", make_shared<Island>("Shell", Point(0, 30), 1000, 200))).first);
	sim_object_map.insert(*island_map.insert(make_pair
	("Bermuda", make_shared<Island>("Bermuda", Point(20, 20)))).first);
	sim_object_map.insert(*island_map.insert(make_pair
	("Treasure_Island", 
		make_shared<Island>("Treasure_Island", Point(50, 5), 100, 5))).first);

	// first insert Ship into ship_map and insert the returned iterator
	// from .insert() back into sim_object_map
	sim_object_map.insert(*ship_map.insert(make_pair
	("Ajax", create_ship("Ajax", "Cruiser", Point(15, 15)))).first);
	sim_object_map.insert(*ship_map.insert(make_pair
	("Xerxes", create_ship("Xerxes", "Cruiser", Point(25, 25)))).first);
	sim_object_map.insert(*ship_map.insert(make_pair
	("Valdez", create_ship("Valdez", "Tanker", Point(30, 30)))).first);
}
...
shared_ptr<Ship_component>
Model::get_ship_composite_ptr(const string& name) const {
...
void Model::describe() const {
	for_each(sim_object_map.cbegin(), sim_object_map.cend(), 
		[](const auto& map_pair) {map_pair.second->describe();});
}

// increment the time, and tell all objects to update themselves
void Model::update() {
	for_each(sim_object_map.cbegin(), sim_object_map.cend(),
		[](const auto& map_pair) {map_pair.second->update();});
...
// has the ship.
void Model::add_ship_to_composite
(const string& composite, shared_ptr<Ship_component>ship) {
...
void Model::add_composite_to_composite
(const string& composite_name, shared_ptr<Ship_component>composite_in) {
...
void Model::remove_ship_from_composite
(const string& composite, const string& ship_name) {
	shared_ptr<Ship_component> composite_ptr 
		= find_composite_ptr(composite);
	...
	cout << it->second->get_name() << 
		" removed from " << composite_ptr->get_name() << endl;
}
...
void Model::attach(shared_ptr<View> view_ptr) {
	...
	for_each(sim_object_map.cbegin(), sim_object_map.cend(), 
		[](const auto& map_pair) {map_pair.second->broadcast_current_state();});

	// Notify View about Ships' information
	for_each(ship_map.cbegin(), ship_map.cend(),
		[](const auto& map_pair) 
...
void Model::notify_location(const string& name, Point location) {
	for_each(view_vec.cbegin(), view_vec.cend(), 
		[&](shared_ptr<View> ptr) {ptr->update_location(name, location);});
}
// notify the views that an object is now gone
void Model::notify_gone(const string& name) {
	for_each(view_vec.cbegin(), view_vec.cend(),
		[&](shared_ptr<View> ptr) {ptr->update_remove(name);});
}
